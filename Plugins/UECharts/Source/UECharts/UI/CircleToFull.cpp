...
#include "CircleToFull.h"

UCircleToFull::UCircleToFull()
{
	
}

void UCircleToFull::OnPopulateMesh(const FGeometry& AllottedGeometry, TArray<FSlateVertex>& vertex, TArray<SlateIndex>& index)
{
	vertex.Empty();
	index.Empty();
	...
	DrawFullCircle(outer, inner, Start, end, 0, 0,false,false);
	/*DrawCircle(outer + 1, outer, Start, end, 0, 0, true,false);
	DrawCircle(inner, inner - 1, Start, end, 0, 0,false,true);*/
	return;
}

void UCircleToFull::DrawFullCircle(float outer, float inner, float start, float end, float offsetX, float offsetY, bool OuterAnti,bool innerAnti)
{
