...
void URoundRectangle::OnPopulateMesh(const FGeometry& AllottedGeometry, TArray<FSlateVertex>& vertex, TArray<SlateIndex>& index)
{
	vertex.Empty();
	index.Empty();
...
void URoundRectangle::SetColor(FSlateVertex pos[])
{
	for (int i = 0; i < 4; i++)
	{
...
void URoundRectangle::DrawAngle(FVector2D centerPos, float startAngle, float endAngle, float curRadius, float stepAngle)
{
