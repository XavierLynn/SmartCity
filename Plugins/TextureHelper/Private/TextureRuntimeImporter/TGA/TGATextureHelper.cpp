...
bool DecompressTGA_Helper(const FTGAFileHeader* TGA, uint32*& TextureData, const int32 TextureDataSize)
{
    if (TGA->ImageTypeCode == 10) // 10 = RLE compressed 
	{
		...
		{
			RTI_LOG(Error, TEXT("TGA uses an unsupported rle-compressed bit-depth: %u"), TGA->BitsPerPixel);
			return false;
		}
	}
	else if (TGA->ImageTypeCode == 2) // 2 = Uncompressed RGB
	{
		...
		{
			RTI_LOG(Error, TEXT("TGA uses an unsupported bit-depth: %u"), TGA->BitsPerPixel);
			return false;
		}
	}
	...
	{
		RTI_LOG(Error, TEXT("TGA is an unsupported type: %u"), TGA->ImageTypeCode);
		return false;
	}
...
bool DecompressTGA(const FTGAFileHeader* TGA, TArray64<uint8>& InBuffer)
{
    ...
    Format = TSF_BGRA8;
    InBuffer.AddUninitialized((int64)TGA->Width * TGA->Height * EMEngine::FTextureImporter::GetBytesPerPixel(Format));

    const int32 TextureDataSize = InBuffer.Num();
    uint32* TextureData = (uint32*)InBuffer.GetData();
