...
private:
    template<typename T>
    static CV_CONSTEXPR std::true_type check1(typename std::is_same<typename std::decay<decltype(std::declval<T>().operator()(std::declval<Args>()...))>::type, Ret>::type*);

    template<typename> static CV_CONSTEXPR std::false_type check1(...);

    typedef decltype(check1<C>(0)) type;
